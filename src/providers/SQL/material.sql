SELECT
  ...
  (
    eg.custo_medio_ponderado * (ea.QTDE_BLOQUEADA + ea.QTDE_DISPONIVEL)
  ) as "Valor"
FROM
  AGH.SCE_ALMOXARIFADOS a
  INNER JOIN AGH.SCE_ESTQ_ALMOXS ea ON (a.seq = ea.alm_seq)
  INNER JOIN AGH.SCO_MATERIAIS m ON (ea.MAT_CODIGO = m.CODIGO)
  INNER JOIN agh.sce_estq_gerais eg ON (
    eg.mat_codigo = m.codigo
    and eg.qtde <> 0
  )
  LEFT JOIN agh.sce_lotes_materiais lm ON (
    lm.mat_codigo = m.codigo
    and lm.seq = ea.lote_seq
  )
WHERE
  (
    (
      COALESCE(ea.qtde_disponivel, 0) + COALESCE(ea.qtde_bloqueada, 0)
    ) > 0
  )
